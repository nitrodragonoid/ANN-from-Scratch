...
void NeuralNetwork::FeedForward(const vector <double> input)
{
    ...
    for (int j = 0; j <input.size();j++) // feeding input to the first layer.
    {
        layers[0].neurons[j].setVal(input[j]);
        //cout << layers[0].neurons[j].getval() << " , ";
    }
...
void NeuralNetwork::BackPropagation(const vector <double> target)
{
    ...
    //cout << sum/outputLayer.neurons.size();
    rms = sqrt((sum/outputLayer.neurons.size())); //calculate root mean square
